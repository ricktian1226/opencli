...
export function parseInstagramPostRef(input) {
    const value = String(input || '').trim();
    if (!value)
        return null;
...
function pickVideoUrl(media) {
    return firstNonEmpty(media?.video_versions?.[0]?.url, media?.carousel_media?.[0]?.video_versions?.[0]?.url);
}
function pickAllImages(media) {
    const values = [
        media?.image_versions2?.candidates?.[0]?.url,
        media?.thumbnail_url,
        media?.display_url,
        ...(Array.isArray(media?.carousel_media)
            ? media.carousel_media.flatMap((item) => [
...
function mediaTypeToName(mediaType) {
...
export function extractPostsFromFeed(feed) {
...
export function extractPostDetailFromMedia(media, fallbackUrl = '') {
    ...
    const caption = String(media?.caption?.text
    ...
    const author = firstNonEmpty(media?.user?.username, media?.owner?.username);
