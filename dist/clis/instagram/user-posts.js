...
cli({
    ...
    `);
        const apiPosts = extractPostsFromFeed(payload?.feed);
        if (apiPosts.length > 0) {
            return apiPosts.slice(0, limit);
        }
        const domPosts = Array.isArray(payload?.domPosts) ? payload.domPosts : [];
        if (domPosts.length > 0) {
            return domPosts.slice(0, limit);
        }
        if (payload?.privateHint) {
            throw new Error('Instagram account is private. Log in with a permitted account and retry.');
        }
        throw new Error(payload?.apiError || 'No Instagram posts found. You may need to log in or Instagram may be blocking this profile.');
    },
});
