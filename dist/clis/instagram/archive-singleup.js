...
import { extractPostDetailFromMedia, extractPostsFromFeed, normalizeInstagramUsername, } from './helpers.js';
async function sleep(seconds) {
    await new Promise((resolve) => setTimeout(resolve, seconds * 1000));
}
async function withRetries(task, attempts = 3, delaySeconds = 2) {
    let lastError;
    for (let index = 0; index < attempts; index += 1) {
        try {
            ...
            lastError = error;
            if (index < attempts - 1)
                await sleep(delaySeconds * (index + 1));
        }
    }
    throw lastError;
}
...
function renderTranslatedParagraph(text) {
    if (!text)
        return '';
    return `
...
function reserveOutputDir(outputRoot, username, shortcode, seen) {
...
async function resolveUserPosts(page, username, limit) {
  ...
  `);
    ...
    if (posts.length === 0) {
        throw new Error(payload?.apiError || `No Instagram posts found for ${username}`);
    }
    return posts;
}
...
function renderWeixinArticle(post, assetFiles, index, assetBase = './') {
...
function writeWeixinHtml(outputDir, post, assetFiles) {
...
function writeRootWeixinHtml(outputRoot, entries) {
...
async function archiveResolvedPost(page, detail, outputRoot, seenDirs) {
