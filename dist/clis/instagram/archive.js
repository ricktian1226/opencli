...
async function downloadViaBrowser(page, url, destPath) {
    try {
    ...
    `);
        if (!payload?.success || !payload?.base64) {
            return { success: false, size: 0, error: String(payload?.error || 'browser fetch failed') };
        }
...
async function resolveSinglePost(page, urlOrShortcode) {
  ...
  `);
    const detail = extractPostDetailFromMedia(payload?.apiMedia || payload?.fallbackMedia, payload?.targetUrl || targetUrl);
    if (!detail)
        throw new Error(payload?.apiError || 'Failed to resolve Instagram post');
    return detail;
}
function writeCaption(outputDir, original, translated) {
    fs.writeFileSync(path.join(outputDir, 'caption.txt'), joinBilingualParagraphs(original, translated), 'utf-8');
}
function writeLinkFiles(outputDir, post, assetFiles) {
...
function renderWeixinArticle(post, assetFiles, index, assetBase = './', options = {}) {
...
function wrapWeixinHtml(title, articles) {
    return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8" />
...
function ensureHtmlFile(htmlPath, html, exportFormat) {
    if (exportFormat === 'pdf' && fs.existsSync(htmlPath) && fs.statSync(htmlPath).size > 0) {
        return;
    }
    fs.writeFileSync(htmlPath, html, 'utf-8');
}
function withPdfTempHtml(htmlPath, html, callback) {
...
function writeWeixinHtml(outputDir, post, assetFiles, exportFormat) {
    ...
    if (exportFormat === 'pdf') {
        const pdfHtml = wrapWeixinHtml(`${post.username} - ${post.shortcode}`, [renderWeixinArticle(post, assetFiles, 1, './', { includeVideos: false })]);
        withPdfTempHtml(htmlPath, pdfHtml, (tempPath) => exportHtmlToPdf(tempPath, path.join(outputDir, 'weixin.pdf')));
    }
}
function writeRootWeixinHtml(outputRoot, entries, exportFormat) {
    const html = wrapWeixinHtml('Instagram Archive', entries.map((entry, index) => renderWeixinArticle(entry.post, entry.assetFiles, index + 1, `./${entry.dirName}/`)));
    const htmlPath = path.join(outputRoot, 'weixin.html');
    ensureHtmlFile(htmlPath, html, exportFormat);
    if (exportFormat === 'pdf') {
        const pdfHtml = wrapWeixinHtml('Instagram Archive', entries.map((entry, index) => renderWeixinArticle(entry.post, entry.assetFiles, index + 1, `./${entry.dirName}/`, { includeVideos: false })));
        withPdfTempHtml(htmlPath, pdfHtml, (tempPath) => exportHtmlToPdf(tempPath, path.join(outputRoot, 'weixin.pdf')));
    }
}
async function archiveSingle(page, request, outputRoot, seenDirs, exportFormat) {
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const exportFormat = String(kwargs['export-format'] || 'html').toLowerCase();
        const raw = fs.readFileSync(inputPath, 'utf-8');
        const requests = parseInputList(raw);
        if (requests.length === 0)
            throw new Error('No valid Instagram URLs found in input file');
        ...
        writeRootWeixinHtml(outputRoot, results.map((item) => ({
            post: item.post,
            dirName: item.dirName,
            assetFiles: item.assetFiles,
        })), exportFormat);
        return results.map(({ post, dirName, assetFiles, ...row }) => row);
    },
});
