...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (!ref)
            throw new Error('Instagram post URL or shortcode is required');
    ...
    `);
        const apiDetail = extractPostDetailFromMedia(payload?.apiMedia, targetUrl);
        if (apiDetail) {
            return [apiDetail];
        }
        const detail = payload?.fallback;
