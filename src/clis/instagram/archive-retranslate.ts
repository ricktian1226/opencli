...
function parseCaptionFile(raw: string): { caption: string; captionZh: string } {
  const blocks = splitParagraphs(raw);
  if (blocks.length === 0) return { caption: '', captionZh: '' };
...
function translateBlockWithCurl(text: string): string {
  ...
  try {
    ...
    return Array.isArray((data as any)?.[0]) ? data[0].map((item: any) => item?.[0] || '').join('').trim() : '';
  } catch {
    return '';
  }
}
...
async function translateParagraphs(page: any, text: string): Promise<string> {
  ...
  for (const block of blocks) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (attempt < 2) await sleep(0.35 * (attempt + 1));
    }
...
function renderMedia(assetBase: string, assetFiles: string[]): string {
...
function renderArticle(post: any, index: number, assetBase = './'): string {
...
function wrapHtml(title: string, articles: string[]): string {
  return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8" />
...
function refreshWeixinForDir(inputDir: string): number {
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    for (const dirName of dirNames) {
      ...
      if (!caption) continue;
      if (captionZh && hasCjk(captionZh)) continue;
      if (translationUnavailable) continue;
