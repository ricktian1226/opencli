...
import { extractPostDetailFromMedia, extractPostsFromFeed, parseInstagramPostRef, type InstagramPostDetail } from './helpers.js';
...
type DownloadablePost = InstagramPostDetail & {
  thumbnail: string;
};
...
function sanitizeCaptionSection(label: string, content: string): string {
  return `${label}\n${content.trim()}\n`;
}

function writeCaptionFile(outputDir: string, caption: string): void {
  const content = [
...
function writeMetadataFile(outputDir: string, post: DownloadablePost, media: DownloadableMedia[]): void {
  fs.writeFileSync(
...
function writeLinkHelperFiles(outputDir: string, post: DownloadablePost, media: DownloadableMedia[]): void {
  const urls = [
...
async function resolveSinglePost(page: any, targetUrl: string, shortcode: string): Promise<DownloadablePost> {
...
async function resolveUserPosts(page: any, username: string, limit: number): Promise<DownloadablePost[]> {
  ...
  if (posts.length === 0) {
    throw new Error((payload as any)?.apiError || 'No Instagram posts found');
  }

  return posts;
}

async function downloadMediaBundle(
  page: any,
  ...
  const media: DownloadableMedia[] = [
    ...
    ...post.video_urls.map((url) => ({ type: 'video' as const, url })),
  ];
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    let posts: DownloadablePost[] = [];

    if (username) {
      posts = await resolveUserPosts(page, username, limit);
    } else {
      const ref = parseInstagramPostRef(String(kwargs.url));
      if (!ref) throw new Error('Provide --username or an Instagram post URL/shortcode');
